  ...
  return (
    <div className="flex h-screen bg-gray-950 overflow-hidden">
      ...
      <aside className="w-52 shrink-0 bg-gray-900 border-r border-gray-800 flex flex-col">
        ...
        <div className="px-5 py-5 border-b border-gray-800">
          <div className="flex items-center gap-2.5">
            <div className="w-7 h-7 rounded-md bg-quinn-500 flex items-center justify-center text-white font-bold text-sm">Q</div>
            <div>
              <div className="text-sm font-semibold text-gray-50 font-serif tracking-tight">Quinn</div>
              <div className="text-[10px] uppercase tracking-widest text-gray-500 mt-0.5">GTM Dashboard</div>
            </div>
          </div>
        </div>
            ...
            <div key={t.id}>
              <button
                ...
                className={`w-full text-left px-5 py-2.5 text-sm font-medium transition-colors ${
                  tab === t.id
                    ? "text-quinn-600 bg-quinn-50 border-r-2 border-quinn-500"
                  ...
                  {GTM_SECTIONS.map((s) => (
                    <button
                      ...
                      className={`w-full text-left pl-4 pr-3 py-1.5 text-xs transition-colors ${
                        activeSection === s.id
                          ? "text-quinn-600 font-medium border-l-2 border-quinn-500 -ml-px"
          ...
          {snapshotMeta ? (
            <div className="text-[10px] text-gray-500 space-y-1.5">
              <div className="flex items-center gap-1.5">
                <span className={`w-1.5 h-1.5 rounded-full shrink-0 ${snapshotMeta.snapshots?.hubspot?.status === "ok" ? "bg-emerald-500" : "bg-amber-400"}`} />
                <span>HubSpot {snapshotMeta.snapshots?.hubspot?.ran_at?.slice(0, 10) || "—"}</span>
              </div>
              <div className="flex items-center gap-1.5">
                <span className={`w-1.5 h-1.5 rounded-full shrink-0 ${snapshotMeta.snapshots?.gong?.status === "ok" ? "bg-emerald-500" : "bg-gray-700"}`} />
                <span>Gong {snapshotMeta.snapshots?.gong?.status === "skipped" ? "not configured" : snapshotMeta.snapshots?.gong?.ran_at?.slice(0, 10) || "—"}</span>
              </div>
              <div className="flex items-center gap-1.5">
                <span className={`w-1.5 h-1.5 rounded-full shrink-0 ${
                  snapshotMeta.snapshots?.quinn?.status === "ok" ? "bg-emerald-500" :
                  snapshotMeta.snapshots?.quinn?.status === "skipped" ? "bg-gray-700" : "bg-amber-400"
                }`} />
                <span>Quinn {snapshotMeta.snapshots?.quinn?.status === "skipped" ? "needs token" :
                       snapshotMeta.snapshots?.quinn?.ran_at?.slice(0, 10) || "—"}</span>
              </div>
            </div>
          ) : (
      ...
      <div className="flex-1 flex flex-col overflow-hidden">
        {/* Header */}
        <header className="shrink-0 z-10 bg-gray-950/80 backdrop-blur border-b border-gray-800 px-8 py-3 flex items-center justify-between gap-4">
          <div className="text-xs text-gray-500 tracking-wide shrink-0">
            Quinn · GTM Dashboard · {new Date().toLocaleDateString("en-US", { month: "long", year: "numeric" })}
          </div>
          <div className="flex items-center gap-4">
            {refreshLabel && (
              <span className="text-[10px] text-gray-600 hidden sm:block uppercase tracking-wide">
            ...
            <button
              ...
              className={`flex items-center gap-1.5 text-xs px-3 py-1.5 rounded-md border transition-colors disabled:opacity-40 ${
                refreshing
                  ...
                  : "border-gray-800 text-gray-400 hover:border-quinn-400 hover:text-quinn-600"
              }`}
            >
              {refreshing ? (
                <>
        ...
        {/* Scrollable content */}
        <main ref={mainRef} className="flex-1 overflow-y-auto overflow-x-hidden bg-gray-950">
