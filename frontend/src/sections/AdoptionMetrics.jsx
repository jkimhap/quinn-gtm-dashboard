...
const LIFECYCLE = {
  power:            { label: "Power",            color: "bg-violet-500",  text: "text-violet-400",  border: "border-violet-500/40",  dot: "bg-violet-400",  desc: "MAU ≥ 30 · Progressions ≥ 500" },
  engaged:          { label: "Engaged",           color: "bg-emerald-500", text: "text-emerald-400", border: "border-emerald-500/40", dot: "bg-emerald-400", desc: "MAU ≥ 10 · Progressions ≥ 100" },
  active:           { label: "Active",            color: "bg-blue-500",    text: "text-blue-400",    border: "border-blue-500/40",    dot: "bg-blue-400",    desc: "Some MAU or course activity"   },
  dormant:          { label: "Dormant",           color: "bg-orange-500",  text: "text-orange-400",  border: "border-orange-500/40",  dot: "bg-orange-400",  desc: "Had usage · MAU now zero"      },
  new:              { label: "New",               color: "bg-amber-500",   text: "text-amber-400",   border: "border-amber-500/40",   dot: "bg-amber-400",   desc: "Joined in last 45 days"        },
...
const ACTION_CONFIG = {
  going_cold:      { label: "Going Cold",       color: "text-red-400",    bg: "bg-red-400/10 border-red-400/20",    icon: "🧊", priority: 0 },
  onboard_needed:  { label: "Needs Onboarding", color: "text-amber-400",  bg: "bg-amber-400/10 border-amber-400/20", icon: "🚀", priority: 1 },
  low_adoption:    { label: "Low Adoption",      color: "text-blue-400",   bg: "bg-blue-400/10 border-blue-400/20",   icon: "📉", priority: 2 },
};
...
function TrendTooltip({ active, payload, label }) {
  ...
  return (
    <div className="bg-gray-900 border border-gray-700 rounded-lg px-3 py-2 text-xs shadow-xl">
      ...
      <div className="text-quinn-300 font-semibold">{fmt(payload[0]?.value)} progressions</div>
      {payload[1] && (
        <div className="text-emerald-400 mt-0.5">{fmt(payload[1]?.value)} new activations</div>
      )}
    </div>
  );
}
...
function StatCard({ label, value, sub, color = "text-white", badge, badgeColor }) {
  return (
    <div className="card">
      <div className="text-xs text-gray-500 mb-2 flex items-center justify-between">
        {label}
        {badge != null && (
          <span className={`text-[10px] px-1.5 py-0.5 rounded-full font-medium ${badgeColor || "bg-gray-800 text-gray-400"}`}>
            {badge}
          </span>
        )}
      </div>
  ...
  return (
    <button
      ...
      className={`text-left p-3 rounded-lg border transition-all ${
        isSelected
          ? `${cfg.border} bg-gray-800/60`
          : "border-gray-800 hover:border-gray-700 bg-gray-900/40"
      }`}
    >
      <div className="flex items-center justify-between mb-1.5">
        <span className={`text-xs font-medium ${cfg.text}`}>{cfg.label}</span>
        <span className={`text-lg font-bold tabular-nums ${isSelected ? cfg.text : "text-white"}`}>
          {data?.count ?? 0}
        </span>
      </div>
    ...
    <div className={`rounded-lg border p-3 ${cfg.bg}`}>
      <div className="flex items-start justify-between gap-2 mb-1.5">
        <div className="min-w-0">
          <div className="text-sm font-medium text-gray-200 truncate">{org.org_name}</div>
      ...
      <div className="flex gap-4 text-xs mt-2">
        <div>
          ...
          <span className={org.progressions > 0 ? "text-gray-300" : "text-gray-700"}>{fmt(org.progressions)}</span>
        </div>
        <div>
          <span className="text-gray-600">MAU </span>
          <span className={org.mau > 0 ? "text-emerald-400" : "text-red-400"}>{fmt(org.mau)}</span>
        </div>
        <div>
        ...
        {org.arr_hs && (
          <div>
            ...
            <span className="text-emerald-400">{fmtArr(org.arr_hs)}</span>
          </div>
        )}
      </div>
...
function Skeleton({ className = "h-8" }) {
  return <div className={`${className} bg-gray-800/60 rounded animate-pulse`} />;
}
  ...
  return (
    <div>
      ...
      <div className="flex items-center justify-between mb-1">
        <h2 className="text-lg font-semibold text-white">Customer Success</h2>
        {meta?.ran_at && (
      ...
      {!configured && (
        <div className="mb-5 flex items-start gap-2 text-xs text-amber-400 bg-amber-400/5 border border-amber-400/20 rounded-lg px-3 py-2">
          <span className="shrink-0 mt-0.5">⚠</span>
          <span>
            Quinn product data not configured. Add{" "}
            <code className="text-amber-300">QUINN_API_TOKEN</code> to Render env vars.
          </span>
        </div>
      )}

      {err && (
        <div className="mb-5 text-red-400 text-xs">Failed to load: {err.message}</div>
      )}
        ...
        {!data ? (
          ...
          <>
            ...
            <StatCard
              ...
              sub="action required"
              color={summary?.action_count > 0 ? "text-red-400" : "text-gray-500"}
      ...
      <div className="mb-6">
        <div className="flex items-center justify-between mb-2">
          <h3 className="text-sm font-semibold text-gray-300">Customer Lifecycle</h3>
          {selectedStage !== "all" && (
            <button
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-4 mb-6">
        ...
        <div>
          <div className="flex items-center justify-between mb-2">
            ...
            <div className="flex gap-3 text-[10px] text-gray-600">
              <span className="flex items-center gap-1"><span className="text-red-400">🧊</span> Going Cold</span>
              <span className="flex items-center gap-1"><span className="text-amber-400">🚀</span> Needs Onboarding</span>
              <span className="flex items-center gap-1"><span className="text-blue-400">📉</span> Low Adoption</span>
            </div>
          </div>
          {!data ? (
        ...
        <div>
          <div className="flex items-center justify-between mb-2">
            <h3 className="text-sm font-semibold text-gray-300">Platform Momentum</h3>
      ...
      <div>
        <div className="flex flex-wrap gap-2 mb-3 items-center">
          ...
          <div className="flex items-center gap-1">
            ...
            <button
              onClick={() => setSelectedStage("all")}
              className={`text-xs px-2 py-1 rounded transition-colors ${selectedStage === "all" ? "bg-quinn-700 text-white" : "text-gray-500 hover:text-gray-300"}`}
            >
              All
            </button>
            {STAGE_ORDER.map(s => {
              ...
              return (
                <button
                  ...
                  className={`text-xs px-2 py-1 rounded transition-colors ${selectedStage === s ? `bg-gray-700 ${cfg.text}` : "text-gray-500 hover:text-gray-300"}`}
                >
                  {cfg.label}
                </button>
              );
          ...
          <div className="flex items-center gap-1 ml-auto">
            ...
            {SORT_OPTIONS.map(s => (
              <button
                ...
                className={`text-xs px-2 py-1 rounded transition-colors ${sortKey === s.key ? "bg-quinn-700 text-white" : "text-gray-500 hover:text-gray-300"}`}
              >
                {s.label}
              </button>
            ))}
                    ...
                    <tr key={o.org_id} className="border-b border-gray-800/40 hover:bg-gray-800/20">
                      <td className="py-1.5 pr-3">
                        <div className="flex items-center gap-1.5 min-w-0">
                          ...
                          <span className="font-medium text-gray-200 truncate max-w-[140px]">{o.org_name}</span>
                        </div>
                      ...
                      <td className="py-1.5 pr-3 text-right tabular-nums">
                        <span className={o.progressions > 500 ? "text-quinn-300 font-medium" : o.progressions > 100 ? "text-gray-300" : o.progressions > 0 ? "text-gray-500" : "text-gray-700"}>
                          {fmt(o.progressions)}
                        </span>
                      </td>
                      ...
                      <td className="py-1.5 pr-3 text-right tabular-nums hidden md:table-cell">
                        <span className={o.learner_pct >= 70 ? "text-emerald-400" : o.learner_pct >= 30 ? "text-amber-400" : o.learner_pct > 0 ? "text-red-400" : "text-gray-700"}>
                          {o.learner_pct > 0 ? `${o.learner_pct}%` : "—"}
                        </span>
                      </td>
                      <td className="py-1.5 pr-3 text-right tabular-nums hidden lg:table-cell">
                        {o.arr_hs
                          ? <span className="text-emerald-400/80">{fmtArr(o.arr_hs)}</span>
