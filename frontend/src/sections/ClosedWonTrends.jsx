...
const AXIS_PROPS = {
  tick: { fontSize: 10, fill: "#9e9890", dy: 4 },
  angle: -45,
  textAnchor: "end",
  interval: 0,
  axisLine: false,
        ...
        <div className="px-3 py-2 border-b border-gray-800">
          ...
          {total > 0 && (
            <div className="text-xs text-gray-400 mt-0.5">
              Total: <span className="text-gray-50 font-medium">{renderDeal ? renderDeal(total, "total") : fmt$(total, { compact: true })}</span>
            </div>
          )}
        </div>
    ...
    return (
      <div style={{ ...ttStyle, minWidth: 200, maxWidth: 280 }}>
        <div className="px-3 py-2 border-b border-gray-800">
          ...
          {avg != null && (
            <div className="text-xs text-gray-400 mt-0.5">
              Avg ACV: <span className="text-gray-50 font-medium">{fmt$(avg, { compact: true })}</span>
  ...
  return (
    <div>
      <div className="flex items-center justify-between mb-4">
        <h2 className="text-lg font-serif font-medium text-gray-50 tracking-tight">Closed-Won Trends (last 12 months)</h2>
        <div className="flex gap-1">
          {STACK_MODES.map(m => (
            <button key={m.key} onClick={() => setMode(m.key)}
              className={`text-xs px-3 py-1.5 rounded transition-colors ${
                mode === m.key ? "bg-quinn-600 text-gray-50" : "bg-gray-800 text-gray-400 hover:text-gray-200"
              }`}>
              {m.label}
            </button>
          ))}
      ...
      {err && <div className="text-red-500 text-sm mb-2">Error: {err.message}</div>}
          ...
          {!data ? (
            ...
            <ResponsiveContainer width="100%" height={260}>
              <BarChart data={countData} margin={MARGIN}>
                <CartesianGrid strokeDasharray="3 3" stroke="#e4ddd5" />
                <XAxis dataKey="label" {...AXIS_PROPS} />
                <YAxis allowDecimals={false} tick={{ fontSize: 10, fill: "#9e9890" }} axisLine={false} tickLine={false} width={24} />
